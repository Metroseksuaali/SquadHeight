...
def _ensure_assets_scanned(registry):
    ...
    try:
        ...
        sh_log.get().warn("scan_paths_synchronous(%s) failed: %s" % (roots, exc))
...
def main():
    output_root = os.path.normpath(os.path.join(_SCRIPT_DIR, "..", "output"))
    log = sh_log.start_session(output_root, "Generating maps_config.json")
    ...
    log.step("%d world assets found under Maps folders" % len(worlds))
    log.phase("Level picks (review these; alternatives are in the log file)")
    ...
    for name, b in bounds.items():
        ...
        log.step("%-12s -> %s" % (name, level))
        for alt in cands[1:6]:
            log.detail("%-12s    (alt: %s)" % (name, alt))
        if len(cands) > 6:
            log.detail("%-12s    (+%d more)" % (name, len(cands) - 6))
    ...
    log.phase("Wrote %s with %d maps" % (out_path, len(maps)))
    if unmatched:
        log.warn("NOT matched (add by hand): %s" % ", ".join(unmatched))
        ...
        folders = sorted({w.split("/")[3] for w in worlds if len(w.split("/")) > 3}
                         - claimed)
        log.step("unclaimed folders under /Game/Maps: %s" % ", ".join(folders))
    log.close()
